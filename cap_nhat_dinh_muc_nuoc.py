...
def ensure_playwright_browsers():
    logging.info(f"=========================================")
    logging.info(f"*** Cập nhật thông tin căn hộ ***")
    logging.info(f"=========================================")
...
def run_test(show_browser=False):
    ...
    data_df = pd.read_excel(excel_path, sheet_name="CanHo", dtype=str)
    ...
    with sync_playwright() as p:
        ...
        logging.info("=========================================")
        logging.info("*** Bắt đầu cập nhật định mức nhân khẩu ***")

        logging.info(f"=========================================")
        logging.info(f"*** Bắt đầu ***")

        for canho, dmnk in data_array:
            logging.info(f"Đang xử lý căn hộ: {canho} - Định mưức nhân khẩu: {dmnk}")  # Thêm log để dễ theo dõi
            ...
            page.locator("//*[@name='waterNorm']").fill(str(dmnk))
