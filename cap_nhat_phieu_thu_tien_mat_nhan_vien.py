...
def ensure_playwright_browsers():
    logging.info(f"=========================================")
    logging.info(f"*** Cập nhật phieu thu tien mat doi voi nhan vien***")
    logging.info(f"=========================================")
...
def run_test(show_browser=False):
    ...
    data_df = pd.read_excel(excel_path, sheet_name="TIENMATNV", dtype=str)
    data_array = list(data_df.itertuples(index=False, name=None))

    logging.info(f"Đọc thông tin đăng nhập: {email} / {password}")
    logging.info(f"Số lượng phiếu thu tiền mặt: {len(data_array)}")
    ...
    with sync_playwright() as p:
        ...
        logging.info("=========================================")
        logging.info("*** Bắt đầu cập nhật phiếu thu tiền mặt của nhân viên ***")

        logging.info(f"=========================================")

        for idx, (ngaythu, tongtien, ten, noidung) in enumerate(data_array, start=1):
            logging.info(f"[{idx}/{len(data_array)}] Số tiền: {tongtien} - Ngày: {ngaythu} - ")
            page.locator("//*[@id='root']/div[2]/main/div/div/div[1]/div/span/div/div[2]/div/div[1]/button").click()
            page.locator("//*[@id='root']/div[2]/main/div/div/div/form/div/div[2]/div/div[2]/div[2]/div").click()
            page.locator("//*[@data-value='employee']").click()
            logging.info(f"Ngày: {str(ngaythu)}")
            ngay_da_dinh_dang = ngaythu.strftime("%d/%m/%Y")
            page.locator("//*[@placeholder='DD/MM/YYYY']").fill(ngay_da_dinh_dang)
            page.locator("//*[@name='amount']").fill(str(tongtien))
            page.locator("//*[@id='root']/div[2]/main/div/div/div/form/div/div[2]/div/div[7]/div[2]/div/input").fill(str(ten))
            page.locator("//*[@id='root']/div[2]/main/div/div/div/form/div/div[2]/div/div[8]/div[2]/div/input").fill("Nhân viên trực vị trí hầm xe")
            page.locator("//*[@name='description']").nth(1).fill(str(noidung))
